...
def main(args):
    ...
    mask_name = str(args.outdir) + '/Mask_' + str(args.pdb).split('/')[-1]
    if not args.extend:
	    os.system('relion_mask_create --i ' + args.pdb + ' --o ' + mask_name)
    else:
        os.system('relion_mask_create --i ' + args.pdb + ' --o ' + mask_name + ' --extend_inimask ' + args.extend)
    return
